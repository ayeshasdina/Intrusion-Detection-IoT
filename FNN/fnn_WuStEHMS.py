...
input_path = '/project/dmani2_uksr/dina_workplace/input-ori-WuStEHMS'
...
### FNN architecture
    model = Sequential()
    model.add(Dense(50, input_dim= 41, 
    	activation="relu"))
    ...
    model.compile(loss=focal_loss(alpha= 2), optimizer= adam,metrics=["accuracy"])	
    return model

model = KerasClassifier(build_fn = create_model,verbose = 0,batch_size = 100,epochs = 100)
...
print("Test WuStEHMS ",report_DT)
print("orininal test WuStEHMS",mcc_ori)
## saving predicted values
y_pred = pd.DataFrame(y_pred)


y_pred.to_csv(os.path.join(output_path,'test-pred-WuStEHMSori.csv'), sep=',')
